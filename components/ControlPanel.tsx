    ...
    return (
        <>
            <div className="flex justify-between items-center mb-2">
                ...
                <button onClick={handlePaste} title="Paste" className="text-slate-400 hover:text-amber-400 transition-colors"><Icon name="clipboard" className="w-5 h-5" /></button>
            </div>
            <textarea
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder={placeholder}
                className="w-full bg-slate-900/50 p-3 rounded-lg h-28 resize-none text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none border border-slate-700/50 text-slate-200 placeholder-slate-500 transition-all"
            />
        </>
    );
};
...
const CreatePanel: React.FC<any> = ({
    ...
    const {
        predefinedReferenceImages, stylePrompts, contextPrompts, lightingPrompts
        ...
        <div className="space-y-6">
            <section>
                ...
                {sourceImage ? (
                    <div className='space-y-3'>
                        <ImageDropzone onImageUpload={handleSourceImageUpload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50 transition-colors"><div className='bg-black/30 rounded-lg p-2'><img src={sourceImageToDataUrl(sourceImage)} alt="Source" className="w-full h-auto object-contain rounded" /></div></ImageDropzone>
                        <button onClick={() => setSourceImage(null)} className='text-red-400 hover:text-red-500 text-sm px-3 py-1.5 rounded-md hover:bg-red-500/10 transition-colors'>{t('delete')}</button>
                    </div>
                ) : (
                    <ImageDropzone onImageUpload={handleSourceImageUpload} className='w-full h-40 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'>
            ...
            <section>
                <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => setShowReferenceGallery(!showReferenceGallery)} className="text-sm text-amber-400 hover:text-amber-300 px-2 py-1 transition-colors">{showReferenceGallery ? t('close') : t('choosePresetImage')}</button>
                </div>
                {showReferenceGallery && (
                    <div className="bg-slate-900/70 p-3 rounded-lg mb-3 border border-slate-700">
                        <div className="flex space-x-1 mb-3 border-b border-slate-700 pb-2 overflow-x-auto thin-scrollbar">
                            {(Object.keys(predefinedReferenceImages) as Array<keyof typeof predefinedReferenceImages>).map(cat => (
                                <button key={cat} onClick={() => setSelectedReferenceCategory(cat)} className={`px-3 py-1.5 text-xs font-semibold capitalize rounded-md whitespace-nowrap transition-colors ${selectedReferenceCategory === cat ? 'bg-amber-500 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>{cat}</button>
                            ))}
                        </div>
                        <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1 thin-scrollbar">
                            {predefinedReferenceImages[selectedReferenceCategory].length > 0 ? (
                                predefinedReferenceImages[selectedReferenceCategory].map(img => <img key={img.url} src={img.url} alt={img.name} onClick={() => handleSetReferenceFromUrl(img.url)} className="w-full h-20 object-cover rounded cursor-pointer hover:ring-2 hover:ring-amber-500 transition-all" />)
                ...
                {isProcessingReference ? (
                    <div className='w-full h-32 border-2 border-dashed border-slate-600 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm animate-pulse'>
                        <p>{t('processingImage')}</p>
                    </div>
                ) : referenceImage ? (
                    <div className="relative group">
                        <ImageDropzone onImageUpload={handleReferenceImageUpload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50"><div className='bg-black/30 rounded-lg p-2'><img src={sourceImageToDataUrl(referenceImage)} alt="Reference" className="w-full h-auto object-contain rounded" /></div></ImageDropzone>
                        <button onClick={() => setReferenceImage(null)} className="absolute top-3 right-3 bg-black/60 rounded-full text-white hover:bg-black/80 p-1 opacity-0 group-hover:opacity-100 z-10 transition-opacity"><Icon name="x-circle" className="w-5 h-5" /></button>
                    </div>
                ) : <ImageDropzone onImageUpload={handleReferenceImageUpload} className='w-full h-32 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'><p>{t('dropzoneHint')}</p></ImageDropzone>}
            </section>
            ...
            <section>
                <div className="flex justify-between items-center mb-2">
                    ...
                    }} title="Paste" className="text-slate-400 hover:text-amber-400 transition-colors"><Icon name="clipboard" className="w-5 h-5" /></button>
                </div>
                <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                ...
                <div className="mt-3 space-y-2">
                    ...
                    <select onChange={(e) => handlePromptSelect(e.target.value, stylePrompts)} value="" className="w-full bg-slate-900/50 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none appearance-none border border-slate-700/50 text-slate-300 transition-all" style={{ background: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e") right 0.5rem center/1.5em 1.5em no-repeat` }}><option value="" disabled>{t('style')}</option>{stylePrompts.map(p => <option key={p} value={p}>{p}</option>)}</select>
                    <select onChange={(e) => handlePromptSelect(e.target.value, contextPrompts)} value="" className="w-full bg-slate-900/50 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none appearance-none border border-slate-700/50 text-slate-300 transition-all" style={{ background: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e") right 0.5rem center/1.5em 1.5em no-repeat` }}><option value="" disabled>{t('context')}</option>{contextPrompts.map(p => <option key={p} value={p}>{p}</option>)}</select>
                    <select onChange={(e) => handlePromptSelect(e.target.value, lightingPrompts)} value="" className="w-full bg-slate-900/50 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none appearance-none border border-slate-700/50 text-slate-300 transition-all" style={{ background: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e") right 0.5rem center/1.5em 1.5em no-repeat` }}><option value="" disabled>{t('lighting')}</option>{lightingPrompts.map(p => <option key={p} value={p}>{p}</option>)}</select>
                </div>
                <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <button
                        onClick={handleGeneratePrompt}
                        disabled={!sourceImage || isGeneratingPrompt || isGeneratingPromptFromText}
                        className="w-full flex items-center justify-center gap-2 font-semibold py-2.5 px-4 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-200 disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed transition-all"
                    >
    ...
    return (
        <div className="space-y-6">
            <section>
                ...
                {sourceImage ? (
                    <div className='space-y-3'>
                        <ImageDropzone onImageUpload={handleSourceImageUpload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50"><div className='bg-black/30 rounded-lg p-2'><img src={sourceImageToDataUrl(sourceImage)} alt="Source" className="w-full h-auto object-contain rounded" /></div></ImageDropzone>
                        <button onClick={() => setSourceImage(null)} className='text-red-400 hover:text-red-500 text-sm px-3 py-1.5 rounded-md hover:bg-red-500/10 transition-colors'>{t('delete')}</button>
                    </div>
                ) : <ImageDropzone onImageUpload={handleSourceImageUpload} className='w-full h-40 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'><div><p>{t('dropzoneHint')}</p><p className="text-xs mt-1 text-slate-500">{t('dropzoneFormats')}</p></div></ImageDropzone>}
            </section>
            ...
            {sourceImage && (
                <section>
                    ...
                    <div className='flex items-center gap-2'>
                        <button onClick={handleToggleSelectingArea} className={`w-full flex items-center justify-center gap-2 font-semibold py-2.5 px-4 rounded-lg transition-colors ${isSelectingArea ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-slate-700 hover:bg-slate-600 text-slate-200'}`}><Icon name="pencil-swoosh" className="w-5 h-5" />{isSelectingArea ? t('cancel') : t('selectArea')}</button>
            ...
            <div className={`${isSelectingArea ? 'opacity-50 pointer-events-none' : ''} transition-opacity space-y-6`}>
                <section>
                    ...
                    <select disabled={isSelectingArea} value={cameraAnglePrompts.some(p => p.value === prompt) ? prompt : ""} onChange={(e) => setPrompt(e.target.value)} className="w-full bg-slate-900/50 p-3 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none appearance-none border border-slate-700/50 text-slate-300" style={{ background: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e") right 0.5rem center/1.5em 1.5em no-repeat` }}><option value="" disabled>{t('selectCameraAnglePlaceholder')}</option>{cameraAnglePrompts.map(p => <option key={p.display} value={p.value}>{p.display}</option>)}</select>
                </section>
                <section>
                    <h3 className="font-semibold text-slate-300 mb-2">4. {t('customDescription')}</h3>
                    <textarea disabled={isSelectingArea} value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder={t('customDescriptionPlaceholder')} className="w-full bg-slate-900/50 p-3 rounded-lg h-24 resize-none text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none border border-slate-700/50 text-slate-200 placeholder-slate-500" />
                </section>
            </div>

            <section>
                <h3 className="font-semibold text-slate-300 mb-2">5. {t('imageCount')}</h3>
                <div className="flex items-center justify-between bg-slate-900/50 rounded-lg p-2 border border-slate-700/50">
                    <button onClick={() => setImageCount((c: number) => Math.max(1, c - 1))} className="px-4 py-2 rounded-md text-xl font-bold hover:bg-slate-700 text-slate-300">-</button>
                    <span className="text-lg font-semibold text-slate-200">{imageCount}</span>
                    <button onClick={() => setImageCount((c: number) => Math.min(10, c + 1))} className="px-4 py-2 rounded-md text-xl font-bold hover:bg-slate-700 text-slate-300">+</button>
                </div>
            </section>
        </div>
    );
        ...
        <div className="space-y-6">
            <section>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                    <button onClick={() => handleSubModeChange('inpaint')} className={`py-2 px-2 text-center rounded-lg border transition-all ${editSubMode === 'inpaint' ? 'bg-amber-500 text-white font-semibold border-amber-500' : 'bg-slate-900/50 hover:bg-slate-700 border-slate-700/50'}`}>{t('editSelectedArea')}</button>
                    <button onClick={() => handleSubModeChange('mergeHouse')} className={`py-2 px-2 text-center rounded-lg border transition-all ${editSubMode === 'mergeHouse' ? 'bg-amber-500 text-white font-semibold border-amber-500' : 'bg-slate-900/50 hover:bg-slate-700 border-slate-700/50'}`}>{t('mergeHouse')}</button>
                    <button onClick={() => handleSubModeChange('mergeMaterial')} className={`py-2 px-2 text-center rounded-lg border transition-all ${editSubMode === 'mergeMaterial' ? 'bg-amber-500 text-white font-semibold border-amber-500' : 'bg-slate-900/50 hover:bg-slate-700 border-slate-700/50'}`}>{t('mergeMaterial')}</button>
                    <button onClick={() => handleSubModeChange('mergeFurniture')} className={`py-2 px-2 text-center rounded-lg border transition-all ${editSubMode === 'mergeFurniture' ? 'bg-amber-500 text-white font-semibold border-amber-500' : 'bg-slate-900/50 hover:bg-slate-700 border-slate-700/50'}`}>{t('mergeFurniture')}</button>
                </div>
            ...
            <section>
                ...
                {sourceImage ? (
                    <div className='space-y-2'>
                        ...
                        <button onClick={() => setSourceImage(null)} className='text-sm text-red-400 hover:text-red-500 w-full text-left px-3 py-1.5 rounded-md hover:bg-red-500/10 transition-colors'>{t('resetImage')}</button>
                    </div>
                ) : <ImageDropzone onImageUpload={handleSourceImageUpload} className='w-full h-40 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'><div><p>{t('dropzoneHint')}</p><p className="text-xs mt-1 text-slate-500">{t('dropzoneFormats')}</p></div></ImageDropzone>}
            </section>
            ...
            {sourceImage && (
                <>
                    {editSubMode === 'inpaint' ? (
                        <>
                            <section>
                                ...
                                <div className="flex bg-slate-900/50 rounded-lg p-1 space-x-1 mb-4 border border-slate-700/50">
                                    <button onClick={() => setEditTool('lasso')} className={`w-1/2 py-2 text-sm rounded-md transition-all ${editTool === 'lasso' ? 'bg-amber-500 text-white font-semibold shadow-sm' : 'text-slate-300 hover:bg-slate-700'}`}>{t('lassoTool')}</button>
                                    <button onClick={() => setEditTool('brush')} className={`w-1/2 py-2 text-sm rounded-md transition-all ${editTool === 'brush' ? 'bg-amber-500 text-white font-semibold shadow-sm' : 'text-slate-300 hover:bg-slate-700'}`}>{t('brushTool')}</button>
                                </div>
                                <button onClick={() => { if (editTool === 'lasso') lassoEditorRef.current?.clear(); else brushEditorRef.current?.clear(); setMaskImage(null); }} className='w-full flex items-center justify-center gap-2 text-sm text-slate-300 hover:text-slate-100 px-3 py-2 rounded-lg bg-slate-700/50 hover:bg-slate-700 transition-colors'><Icon name="arrow-uturn-left" className="w-4 h-4" />{t('clearSelection')}</button>
                                <div className='mt-4 space-y-2'>
                                    ...
                                    <input id="brushSize" type="range" min="1" max={editTool === 'lasso' ? 10 : 50} value={brushSize} onChange={(e) => setBrushSize(Number(e.target.value))} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-amber-500" />
                                </div>
                            </section>
                            <section>
                                ...
                                {editReferenceImage ? (
                                    <div className="relative group">
                                        <ImageDropzone onImageUpload={handleEditReferenceImageUpload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50"><div className='bg-black/30 rounded-lg p-2'><img src={sourceImageToDataUrl(editReferenceImage)} alt="Edit Reference" className="w-full h-auto object-contain rounded" /></div></ImageDropzone>
                                        <button onClick={() => setEditReferenceImage(null)} className="absolute top-3 right-3 bg-black/60 rounded-full text-white hover:bg-black/80 p-1 opacity-0 group-hover:opacity-100 z-10 transition-opacity"><Icon name="x-circle" className="w-5 h-5" /></button>
                                    </div>
                                ) : <ImageDropzone onImageUpload={handleEditReferenceImageUpload} className='w-full h-32 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'><p>{t('dropzoneHint')}</p></ImageDropzone>}
                            </section>
                            <section>
                        ...
                        <>
                            <section>
                                ...
                                {sourceImage2 ? (
                                    <div className='space-y-3'>
                                        <ImageDropzone onImageUpload={handleSourceImage2Upload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50"><div className='bg-black/30 rounded-lg p-2'><img src={sourceImageToDataUrl(sourceImage2)} alt="Source 2" className="w-full h-auto object-contain rounded" /></div></ImageDropzone>
                                        <button onClick={() => setSourceImage2(null)} className='text-red-400 hover:text-red-500 text-sm px-3 py-1.5 rounded-md hover:bg-red-500/10 transition-colors'>{t('delete')}</button>
                                    </div>
                                ) : <ImageDropzone onImageUpload={handleSourceImage2Upload} className='w-full h-32 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'><div><p>{t('dropzoneHint')}</p></div></ImageDropzone>}
                            </section>
                            <section>
                                ...
                                {(editSubMode === 'mergeMaterial' || editSubMode === 'mergeFurniture') && (
                                    <div className="mt-3">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => setPrompt(e.target.value)}
                                            className="w-full bg-slate-900/50 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none appearance-none border border-slate-700/50 text-slate-300"
                                            style={{ background: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e") right 0.5rem center/1.5em 1.5em no-repeat` }}
                                        >
                    ...
                    <section>
                        <h3 className="font-semibold text-slate-300 mb-2">5. {t('imageCount')}</h3>
                        <div className="flex items-center justify-between bg-slate-900/50 rounded-lg p-2 border border-slate-700/50">
                            <button onClick={() => setImageCount((c: number) => Math.max(1, c - 1))} className="px-4 py-2 rounded-md text-xl font-bold hover:bg-slate-700 text-slate-300 transition-colors">-</button>
                            <span className="text-lg font-semibold text-slate-200">{imageCount}</span>
                            <button onClick={() => setImageCount((c: number) => Math.min(10, c + 1))} className="px-4 py-2 rounded-md text-xl font-bold hover:bg-slate-700 text-slate-300 transition-colors">+</button>
                        </div>
                    </section>
                </>
            )}
    ...
    return (
        <div className="space-y-6">
            <section>
                ...
                {sourceImage ? (
                    <div className='space-y-3'>
                        <ImageDropzone onImageUpload={handleSourceImageUpload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50"><div className='bg-black/30 rounded-lg p-2'><img src={sourceImageToDataUrl(sourceImage)} alt="Floor plan" className="w-full h-auto object-contain rounded" /></div></ImageDropzone>
                        <button onClick={() => setSourceImage(null)} className='text-red-400 hover:text-red-500 text-sm px-3 py-1.5 rounded-md hover:bg-red-500/10 transition-colors'>{t('delete')}</button>
                    </div>
                ) : <ImageDropzone onImageUpload={handleSourceImageUpload} className='w-full h-40 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'><div><p>{t('dropzoneHint')}</p><p className="text-xs mt-1 text-slate-500">{t('dropzoneFormats')}</p></div></ImageDropzone>}
            </section>
            <section>
                ...
                <div className="flex bg-slate-900/50 rounded-lg p-1 space-x-1 border border-slate-700/50">
                    <button onClick={() => handleModeChange('render')} className={`w-1/2 py-2 text-sm rounded-md transition-all ${planTo3dMode === 'render' ? 'bg-amber-500 text-white font-semibold shadow-sm' : 'text-slate-300 hover:bg-slate-700'}`}>{t('create3DImage')}</button>
                    <button onClick={() => handleModeChange('colorize')} className={`w-1/2 py-2 text-sm rounded-md transition-all ${planTo3dMode === 'colorize' ? 'bg-amber-500 text-white font-semibold shadow-sm' : 'text-slate-300 hover:bg-slate-700'}`}>{t('colorizePlan')}</button>
                </div>
            </section>
            {planTo3dMode === 'render' && (
                <section>
                    ...
                    {isProcessingReference ? (
                        ...
                        <div className="relative group">
                            <ImageDropzone onImageUpload={handlePlanReferenceImageUpload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50"><div className='bg-black/30 rounded-lg p-2'><img src={sourceImageToDataUrl(referenceImage)} alt="Reference" className="w-full h-auto object-contain rounded" /></div></ImageDropzone>
                            <button onClick={() => setReferenceImage(null)} className="absolute top-3 right-3 bg-black/60 rounded-full text-white hover:bg-black/80 p-1 opacity-0 group-hover:opacity-100 z-10 transition-opacity"><Icon name="x-circle" className="w-5 h-5" /></button>
                        </div>
                    ) : <ImageDropzone onImageUpload={handlePlanReferenceImageUpload} className='w-full h-32 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'><p>{t('dropzoneHint')}</p></ImageDropzone>}
                </section>
            )}
            <section>
                ...
                <div className="mt-3 space-y-2">
                    <select onChange={(e) => setPrompt(prompt + ", " + e.target.value)} value="" className="w-full bg-slate-900/50 p-2.5 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:outline-none appearance-none border border-slate-700/50 text-slate-300 transition-all" style={{ background: `url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%2394a3b8' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e") right 0.5rem center/1.5em 1.5em no-repeat` }}>
...
const VideoPanel: React.FC<any> = ({ sourceImage, handleSourceImageUpload, setSourceImage, prompt, setPrompt }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <section>
                <h3 className="font-semibold text-slate-300 mb-2">2. {t('motionDescription')}</h3>
    ...
    return (
        <div className="space-y-6">
            <section>
                ...
                {sourceImage ? (
                    <div className='space-y-3'>
                        <ImageDropzone onImageUpload={handleSourceImageUpload} className="cursor-pointer rounded-lg relative group bg-black/30 border border-slate-700/50 hover:border-amber-500/50">
                        ...
                        <button onClick={() => { setSourceImage(null); setCanvaObjects([]); setCanvaObjectTransforms([]); setSelectedCanvaObjectIndex(null); }} className='text-red-400 hover:text-red-500 text-sm px-3 py-1.5 rounded-md hover:bg-red-500/10 transition-colors'>{t('deleteAll')}</button>
                    </div>
                ) : (
                    <ImageDropzone onImageUpload={handleSourceImageUpload} className='w-full h-40 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'>
                        <div><p>{t('dropzoneHint')}</p><p className="text-xs mt-1 text-slate-500">{t('dropzoneFormats')}</p></div>
            ...
            {sourceImage && (
                <>
                    <section>
                        ...
                        <ImageDropzone onImagesUpload={handleDecorUpload} multiple={true} className='w-full h-24 border-2 border-dashed border-slate-600 hover:border-amber-500 rounded-lg flex items-center justify-center text-center text-slate-400 text-sm cursor-pointer transition-colors bg-slate-900/30 hover:bg-slate-900/50'>
                            ...
                            <div className="mt-3 grid grid-cols-4 gap-2">
                                {canvaObjects.map((obj: SourceImage, i: number) => (
                                    <img
                                        ...
                                        title={t('clickToAdd')}
                                        className={`w-full h-16 object-contain rounded-md cursor-pointer bg-slate-700/50 p-1 hover:ring-2 hover:ring-amber-400 transition-all`}
                                    />
                                ))}
                            </div>
                        )}
                    ...
                    <section>
                        ...
                        <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-lg border border-slate-700/50">
                            <label htmlFor="lock-layout" className="text-sm flex items-center gap-2 cursor-pointer text-slate-300">
                                <input
                                    type="checkbox"
                                    id="lock-layout"
                                    checked={isCanvaLayoutLocked}
                                    onChange={(e) => setIsCanvaLayoutLocked(e.target.checked)}
                                    className="w-4 h-4 rounded text-amber-600 bg-slate-700 border-slate-600 focus:ring-amber-500"
                                />
                                {t('lockLayout')}
                            </label>
                        ...
                        {selectedTransform && !isCanvaLayoutLocked && (
                            <div className="mt-3 space-y-3 p-3 bg-slate-900/50 rounded-lg border border-slate-700/50">
                                <div>
                                    <label className="text-sm text-slate-300">{t('rotate')}: {selectedTransform.rotation}°</label>
                                    <input type="range" min="0" max="360" value={selectedTransform.rotation} onChange={(e) => updateSelectedObjectTransform({ rotation: Number(e.target.value) })} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-amber-500" />
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => updateSelectedObjectTransform({ flipHorizontal: !selectedTransform.flipHorizontal })} className="w-full text-sm py-2 px-3 bg-slate-700 hover:bg-slate-600 rounded-md text-slate-200 transition-colors">{t('flipHorizontal')}</button>
                                    <button onClick={() => updateSelectedObjectTransform({ flipVertical: !selectedTransform.flipVertical })} className="w-full text-sm py-2 px-3 bg-slate-700 hover:bg-slate-600 rounded-md text-slate-200 transition-colors">{t('flipVertical')}</button>
                                </div>
                            </div>
                        )}
                    </section>
    ...
    return (
        <div className="space-y-6">
            <section>
                ...
                {sourceImage ? (
                    <div className='space-y-3'>
                        <ImageDropzone onImageUpload={handleSourceImageUpload} className="cursor-pointer rounded-lg border border-slate-700/50 hover:border-amber-500/50">
...
export const ControlPanel: React.FC<any> = (props) => {
    ...
    return (
        <div className="lg:col-span-4 xl:col-span-3 bg-[#1e293b] p-5 rounded-2xl flex flex-col gap-5 h-max shadow-2xl shadow-slate-900/50 border border-slate-700/50 backdrop-blur-sm">
            {renderPanel()}

            <button onClick={handleGeneration} disabled={isGenerationDisabled()} className="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 disabled:bg-slate-700 disabled:text-slate-500 disabled:cursor-not-allowed mt-4 text-base shadow-lg shadow-amber-500/20 hover:shadow-amber-500/40 transition-all transform hover:scale-[1.02] active:scale-[0.98]">
